...
public class ch1132_JDBCJTable extends JFrame {
	...
	public void initTablModel() {
		...
		tm = new AbstractTableModel() {
			...
			public Class getColumnClass(int column) {
				...
				return colClasses[column]; 	//设置表格列名
			}
	...
	public void showData() throws SQLException, ClassNotFoundException {
		...
		Connection connection = (Connection) DriverManager.getConnection(connString);
		...
		for( int i=0; i<colCnt; i++ ) {
			...
			String className = meta.getCatalogName(i+1);
	...
	public static void main(String[] args) {
		...
		SwingUtilities.invokeLater(()->{
			ch1132_JDBCJTable f = new ch1132_JDBCJTable();
			f.initTablModel();
			try {
				f.showData();
			}catch( Exception e ) { e.printStackTrace(); }
			f.setSize( 400,300 );
			f.setTitle( "Show Database table in JTable（在 JTable 中显示数据库表）" );
