...
public class ch1061_Clock2 extends JApplet implements Runnable {
	...
	public void init() {
		...
		try {
			handColor = new Color(Integer.parseInt(getParameter("fgcolorl"), 16));
		} catch(Exception ex) {ex.printStackTrace();}
		try {
			numberColor = new Color(Integer.parseInt(getParameter("fgcolor2"), 16));
		} catch(Exception ex) {ex.printStackTrace();}
		
		resize(400, 300); 	// Set clock window size
	}
	...
	g.drawString("3", xcenter+45, ycenter+3);
	g.drawString("2", xcenter-5, ycenter-37);
	...
	public static void main(String[] args) {
		...
		JFrame f = new JFrame();
		f.setSize(400, 300);
		ch1061_Clock2 p = new ch1061_Clock2();
		f.add(p);
		p.init();
		p.start();
