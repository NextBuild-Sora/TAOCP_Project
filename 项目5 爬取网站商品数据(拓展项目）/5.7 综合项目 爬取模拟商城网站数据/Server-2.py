    ...
    try:
        ...
        for i in range(1,count):
            ...
            if rowIndex>=startRow and rowIndex<endRow:
                row=row.strip().strip("\n")
                s=row.split(",")
                phones.append({"no": s[0], "mark": s[1], "price": s[2], "note": s[3], "image": s[4]})
            rowIndex+=1
        fobj.close()
    except Exception as err:
        print(err)
    return flask.render_template("phone.html",phones=phones,rowItemCount=rowItemCount,numbers=len(phones),pageIndex=pageIndex,pageCount=pageCount) 
